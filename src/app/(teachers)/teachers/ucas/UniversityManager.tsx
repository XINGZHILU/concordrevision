...
/**
 * Component for managing universities in the teacher dashboard
 * Allows teachers to create, edit, and delete universities
 */
export function UniversityManager({ universities, ucasSubjects }: { universities: UniversityWithCourses[], ucasSubjects: UCASSubject[] }) {
  ...
  // Auto-generate university ID from name
  useEffect(() => {
  ...
  async function handleCreateUniversity() {
    ...
    const response = await fetch('/api/teachers/universities', {
      method: 'POST',
  ...
  async function handleDeleteUniversity(universityId: string) {
    const response = await fetch(`/api/teachers/universities/${universityId}`, {
      method: 'DELETE',
    });
        ...
        {universities.map((uni) => (
          <div key={uni.id} className="flex items-center justify-between p-3 bg-muted/50 rounded-lg">
            ...
            <div className="flex items-center gap-2">
              <Link href={`/teachers/ucas/universities/${uni.id}`} passHref>
