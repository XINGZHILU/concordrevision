...
/**
 * Form component for managing admission statistics for universities
 * Allows teachers to add, edit, and delete admission stats
 */
export function AdmissionStatsForm({ universityId, stats }: { universityId: string, stats: AdmissionStats[] }) {
  ...
  async function onSubmit(values: z.infer<typeof formSchema>) {
    const url = isEditing
      ? `/api/teachers/admission-stats/${isEditing}`
      : `/api/teachers/universities/${universityId}/stats`;
  ...
  const handleDelete = async (statId: number) => {
    ...
    const response = await fetch(`/api/teachers/admission-stats/${statId}`, {
        method: 'DELETE',
    });
