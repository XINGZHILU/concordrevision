...
export default function UCASUploadForm({ universities, author }: { universities: University[], author: string }) {
    ...
    useEffect(() => {
        ...
        if (saved) {
            const { title, content, postType, selectedUniversity, selectedCourse } = JSON.parse(saved);
            setTitle(title || "");
            setContent(content || "");
            setPostType(postType || 0);
    ...
    useEffect(() => {
        const data = JSON.stringify({ title, content, postType, selectedUniversity, selectedCourse });
        sessionStorage.setItem(storageKey, data);
    }, [title, content, storageKey, postType, selectedUniversity, selectedCourse]);
    ...
    async function upload(files: File[], title: string, desc: string) {
        ...
        try {
            ...
            toast({
                ...
                description: "The post will be available for view after being approved.",
            });
    ...
    async function store(event: React.FormEvent) {
        ...
        await upload(stagedFiles, title, content);
    }
            ...
            <div className="w-full mx-auto bg-card rounded-lg shadow p-6">
                ...
                <form className="space-y-6" onSubmit={store}>
                    ...
                    <div>
                        ...
                        <RadioGroup value={postType.toString()} onValueChange={(v) => setPostType(parseInt(v))} className="mt-2">
                    ...
                    {(postType === 1 || postType === 2) && (
                        <Select onValueChange={handleUniversityChange} value={selectedUniversity || undefined}>
                            <SelectTrigger>
                    ...
                    {postType === 2 && selectedUniversity && (
                        <Select onValueChange={(v) => setSelectedCourse(v)} value={selectedCourse || undefined}>
                            <SelectTrigger>
